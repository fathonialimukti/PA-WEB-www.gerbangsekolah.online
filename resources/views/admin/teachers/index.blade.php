    ...
    <div class="roles-permissions">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-5/12 px-4 py-3">Assigned Grade</div>
            ...
            @foreach ($teachers as $teacher)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $teacher->subject }}</div>
                    <div class="w-5/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">
                        @foreach ($teacher->grades as $grade)
                            <span class="bg-gray-200 text-sm mr-1 mb-1 px-2 font-semibold border rounded-full">{{ $grade->class_name }}</span>
                        @endforeach
                    </div>
                    <div class="w-3/12 flex items-center justify-end px-3">
                        <a href="{{ route('teacher.show',$teacher) }}" class="ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm" >
                        ...
                        <form action="{{ route('teacher.destroy',$teacher->id) }}" id="delete-teacher" method="POST" class="inline-flex ml-1" >
                            @csrf
                            @method('DELETE')
                            <button type="button" class="bg-gray-600 block p-1 border border-gray-600 rounded-sm " onclick="deleteConfirm('delete-teacher','{{ $teacher->user->name }}')">
        ...
        @include('admin.modals.delete',['name' => 'teacher'])
    </div>
@endsection
